...
function ReportModal({ item, onClose, onEdit, activeTab }: ReportModalProps) {
  const handleDelete = async (id: string) => {
    if (!confirm("Tem certeza que deseja excluir este item?")) return;

    try {
      const endpoint = activeTab === "pdf" ? "/api/reports/pdfs" : "/api/reports/videos";
      ...
      <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg shadow-lg max-w-md w-full p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">
            Opções do {activeTab === "pdf" ? "PDF" : "Vídeo"}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            <span>Editar {activeTab === "pdf" ? "PDF" : "Vídeo"}</span>
          </button>
          ...
          {item.url && (
            <a
              ...
              className="flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-all duration-200 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-400/30 hover:border-purple-400/50 text-white"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          >
            ...
            <span>Excluir {activeTab === "pdf" ? "PDF" : "Vídeo"}</span>
          </button>
        </div>
      </div>
    </div>
